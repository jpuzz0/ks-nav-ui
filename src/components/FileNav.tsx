...
export const FileNav = () => {
  ...
  const selectedInstanceVersion = selectedInstance && `${selectedInstance.version} (${selectedInstance.note})`;
  ...
  const navItemDictionary: Record<string, string[]> = files.reduce((acc, { name: path }: File) => {
    ...
    filePathParts.reduce(
      (prev: any, pathPart, i) =>
        (prev[pathPart] =
          filePathParts.length - i - 1 ? prev[pathPart] || {} : (prev[pathPart] || []).concat([fileName])),
      acc,
    );
